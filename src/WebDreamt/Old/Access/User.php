...
class User extends Object {
	...
	protected function __construct() {
		...
		if ((self::SESSION && !isset($_SESSION['_thetoken'])) || (!self::SESSION && !isset($_COOKIE['_thetoken']))) {
			$this->userData = null;
			return;
		}
	...
	public function login($username, $password, $force = false) {
		...
		if (!empty($r[0][self::LOCKOUT]) && strtotime($r[0][self::LOCKOUT]) < time() - self::LOCKOUT_TIME) {
			$login_error_count = 0;
		}
		...
		if ((crypt($password, self::SALT_PREFIX . $hashed_password) !== self::SALT_PREFIX .
			...
			$q = $this->userQB->update(array(self::LOGIN_ERROR_FIELD => $login_error_count,
				self::LOCKOUT => date("Y-m-d H:i:s")), array(self::USERNAME_FIELD => $username));
