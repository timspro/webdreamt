 ...
 * @method     ChildGroupsQuery orderByPermissions($order = Criteria::ASC) Order by the permissions column
 ...
 * @method     ChildGroupsQuery groupByPermissions() Group by the permissions column
 ...
 *
 * @method     ChildGroupsQuery leftJoinUsersGroups($relationAlias = null) Adds a LEFT JOIN clause to the query using the UsersGroups relation
 * @method     ChildGroupsQuery rightJoinUsersGroups($relationAlias = null) Adds a RIGHT JOIN clause to the query using the UsersGroups relation
 * @method     ChildGroupsQuery innerJoinUsersGroups($relationAlias = null) Adds a INNER JOIN clause to the query using the UsersGroups relation
 *
 * @method     \UsersGroupsQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 *
 * @method     ChildGroups findOneById(int $id) Return the first ChildGroups filtered by the id column
 * @method     ChildGroups findOneByName(string $name) Return the first ChildGroups filtered by the name column
 * @method     ChildGroups findOneByPermissions(string $permissions) Return the first ChildGroups filtered by the permissions column
 * @method     ChildGroups findOneByCreatedAt(string $created_at) Return the first ChildGroups filtered by the created_at column
 * @method     ChildGroups findOneByUpdatedAt(string $updated_at) Return the first ChildGroups filtered by the updated_at column
 *
 ...
 * @method     ChildGroups[]|ObjectCollection findByPermissions(string $permissions) Return ChildGroups objects filtered by the permissions column
...
abstract class GroupsQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, name, permissions, created_at, updated_at FROM groups WHERE id = :p0';
        try {
    ...
    public function filterByPermissions($permissions = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(GroupsTableMap::COL_PERMISSIONS, $permissions, $comparison);
    }
    ...
    /**
     * Filter the query by a related \UsersGroups object
     *
     * @param \UsersGroups|ObjectCollection $usersGroups  the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildGroupsQuery The current query, for fluid interface
     */
    public function filterByUsersGroups($usersGroups, $comparison = null)
    {
        if ($usersGroups instanceof \UsersGroups) {
            return $this
                ->addUsingAlias(GroupsTableMap::COL_ID, $usersGroups->getGroupId(), $comparison);
        } elseif ($usersGroups instanceof ObjectCollection) {
            return $this
                ->useUsersGroupsQuery()
                ->filterByPrimaryKeys($usersGroups->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByUsersGroups() only accepts arguments of type \UsersGroups or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the UsersGroups relation
     *
     ...
     * @return $this|ChildGroupsQuery The current query, for fluid interface
     */
    public function joinUsersGroups($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('UsersGroups');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'UsersGroups');
        }
    ...
    /**
     * Use the UsersGroups relation UsersGroups object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \UsersGroupsQuery A secondary query class using the current class as primary query
     */
    public function useUsersGroupsQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinUsersGroups($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'UsersGroups', '\UsersGroupsQuery');
    }
